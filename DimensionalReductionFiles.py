...
def LDA(data, labels, alpha=0, nvars=10):
    ...
    sinds = np.argsort(np.abs(np.diag(L)))[::-1]  
    L = L[sinds]
    U = U[:, sinds]
...
def sLDA(data, target, slices=30, alpha=0, nvar=10):
    ...
    labels = np.zeros([1, N])  
    l = int(np.ceil(N / slices)) #  the number of data points in each slice, 
    for i in range(slices - 1):
        labels[0, i * l:(i + 1) * l] = i
    labels[0,(slices - 1) * l:] = slices-1
    
    sinds = np.argsort(target, axis=0)  # sorts the target array in ascending order and returns the sorted indices. 
    sinv = np.zeros_like(sinds)
    sinv[sinds[:,0]] = np.arange(len(sinds)).reshape(-1,1)  # inverse permutation of the sorted indices.
    labels = labels[0,sinv[:,0:1].astype(int)].T  # applies the inverse permutation to the labels array
